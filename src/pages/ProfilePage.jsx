...
export function ProfilePage({ onNavigate, currentUser, activePortalId }) {
  ...
  const isFacultyUser = activePortalId === 'faculty' ||
    ...
    currentUser?.id?.startsWith('FAC');

  if (isFacultyUser) {
    return <FacultyProfileView user={currentUser} onNavigate={onNavigate} />;
  }

  const isCollegeUser = activePortalId === 'college' ||
    currentUser?.role?.toLowerCase().includes('dean') ||
    currentUser?.id?.startsWith('AISHE');

  if (isCollegeUser) {
    return <CollegeProfileView user={currentUser} onNavigate={onNavigate} />;
  }

  const isMinistryUser = activePortalId === 'admin' ||
    currentUser?.role?.toLowerCase().includes('ministry') ||
    currentUser?.id?.startsWith('CCRAS');

  if (isMinistryUser) {
    return <MinistryProfileView user={currentUser} onNavigate={onNavigate} />;
  }
